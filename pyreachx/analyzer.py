...
class CodeAnalyzer:
    ...
    def _should_analyze_file(self, file_path: Path) -> bool:
        ...
        return not any(pattern.match(relative_path) for pattern in self.config.exclude_patterns)
                    ...
                    if isinstance(node.value.func, ast.Name):
                        ...
                        for target in node.targets:
                            if isinstance(target, ast.Name):
                                ...
                                self.analyzer.class_instances.setdefault(instance_name, []).append(class_name)
                self.generic_visit(node)
